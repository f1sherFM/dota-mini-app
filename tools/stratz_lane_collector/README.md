...
- safe lane (`POSITION_1` + `POSITION_5`), mid (`POSITION_2`) и
  off lane (`POSITION_3` + `POSITION_4`);
- `matchCount`, обычные W/D/L, stomp W/L, победы в матче и CS.

Строки разных недель и рангов суммируются по направленной паре
`heroId1 → heroId2` и типу линии. Никакой производный lane score в файл пока не
